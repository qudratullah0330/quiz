...
export default function NewCompaign({navigation}) {
 const[show,setshow]=useState(false)
 const [Dateshow,setDate]=useState('')
 const [showLocation,setShowLocation]=useState()
 const [Volunteers,setVolunteers]=useState()
 const [CollectedBlood,setColletedBlood]=useState()
...
const PostRequset = async () => {
  try {
    ...
    const docRef = await addDoc(collection(db, 'Compaign'), {
    ...
    if(DateshowError||LocationError||BloodError||VolunteersError){

    }
    else{
      navigation.navigate('Compaign')
    }
  ...
  return (
   ...
   <View  style={{flex:1,backgroundColor:'white'}}>
      <View style={styles.imgView}>
        <ImageBackground source={require('../Img/inputIcon/img9.png')} style={{marginRight:22,width:'65%',height:170,alignItems:'center',justifyContent:'center'}}>
...
/>
      ...
      {LocationError?<Text style={{color:'red',marginRight:'44%'}}>Please enter valid Location</Text>:null}
      ...
      <View style={styles.inputContainerStyle}>
            ...
            <TextInput style={{width:'60%',fontSize:18,color:'#B50102'}} 
             value={showLocation} onChangeText={(text)=>setShowLocation(text)}
             placeholder='Enter Location'
            placeholderTextColor={'#B50102'}/>
         </View>
         {VolunteersError?<Text style={{color:'red',marginRight:'36%'}}>enter valid number Volunteeers</Text>:null}
         
         <View style={styles.inputContainerStyle}>
            <Image style={styles.imgStyle} source={require('../Img/inputIcon/img9.png')}/>
            <View style={styles.verticleLine}></View>
            <TextInput style={{width:'60%',fontSize:18,color:'#B50102'}} value={Volunteers}
            onChangeText={(text)=>setVolunteers(text)}
            placeholder='Enter No Of Volunteers'placeholderTextColor={'#B50102'}/>
         </View>
         {BloodError?<Text style={{color:'red',marginRight:'26%'}}>enter valid number Collected Blood</Text>:null}
         <View style={styles.inputContainerStyle}>
            ...
            value={CollectedBlood} onChangeText={(text)=>setColletedBlood(text)} placeholder='Collected Blood Amount'
             placeholderTextColor={'#B50102'}/>
         </View>
{DateshowError?<Text style={{color:'red',marginRight:'46%'}}>Please enter corect Date</Text>:null}
         <View style={styles.inputContainerStyle}>
            <Image style={styles.imgStyle} source={require('../Img/inputIcon/img11.png')}/>
            <View style={styles.verticleLine}></View>
            <TextInput style={{width:'60%',fontSize:18,color:'#B50102'}}
            value={Dateshow} onPressIn={()=>setshow(true)} placeholder='Select Date'placeholderTextColor={'#B50102'}/>
         </View>
